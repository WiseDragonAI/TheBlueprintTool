...
20. **Operator reply boundary:** The gate prompt receives the canonical thread identity and uses `ledger-cli answer` for its operator-facing response. Its output card remains the direct handoff for the next queued skill.
21. **Failure propagation:** Dynamic successor executions retain the external predecessor identity in durable topology. Failure, cancellation, interruption, and restart recovery cancel every queued descendant across the originating and successor runs.
