...
6. `<LAST_AGENT_NOTE>`
   1. **WHAT:** Supplies only the latest agent note from the active admitted thread, rendered as canonical thread Markdown.
   2. **WHY:** Authored prompts can receive the immediately preceding agent answer without replaying the complete conversation.
7. `<FILE_MAP>`
