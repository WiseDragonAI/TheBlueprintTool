...
2. [Codex content authoring](./codex-content-authoring.md) defines owner-scoped skill, prompt, and Task-card Markdown APIs; direct Markdown routing; exact-byte Git recovery; cursor history; persistent CodeMirror sessions; versioned prompt compilation for `SYSTEM_PROMPT`, `SKILL`, and `CODEX_RUN`; and federation publication.
3. [Prompt expression and system-prompt catalog](./prompt-expression-catalog.md) defines every prompt-reference syntax, runtime expression, provider purpose, and mandatory system prompt.
4. [Epoch-3 task state and federation](./epoch-3-task-state-and-federation.md) is the historical architecture and rollback reference for offline migration, structural entities, immutable content, durability, relay anti-entropy, derived remote stores, lazy content routing, and production diagnostics.
5. [Epoch-4 task assignment, execution, and content](./epoch-4-task-assignment-execution-and-content.md) defines durable assignment, replicated execution, thread-note and Markdown consistency, immutable execution artifacts, in-place media references, and the recoverable node migration transaction.
6. [Epoch-4 federation repair and recovery](./epoch-4-federation-repair-and-recovery.md) defines bounded relay-owned repair, watcher admission, collision evidence, explicit paused-store recovery, and structural convergence proof.
7. [Frontend telemetry and incident containment](./frontend-telemetry-and-incident-containment.md) defines local browser diagnostic transport, resource bounds, stopped-operation classification, and safe normalization of the legacy false-pause scope.
