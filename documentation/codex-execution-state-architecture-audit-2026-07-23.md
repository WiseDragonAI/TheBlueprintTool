...
1. **Historical status:** This audit records the pre-Epoch-4 execution model and its activation incidents. Its architecture findings are superseded by [Epoch-4 task assignment, execution, and content](./documentation/architecture/epoch-4-task-assignment-execution-and-content.md).
2. **Decision OS had no single authoritative Codex execution record.** One visible run was split across replicated task fields, a direct-process queue, a pipeline manifest, in-memory runtime state, the operating-system process, log files, output Markdown, session-history fields, the Control Room cache, and frontend optimistic state.
3. **The screenshot was a real state-ordering defect, not only a missing timer style.** The pipeline manifest and operating-system process could become `running` before the replicated task projection was durably changed from `queued` to `running`. The process-start callback then invalidated the Control Room immediately. That invalidation could rebuild the old queued projection, and completion of the asynchronous task-state write did not publish another invalidation.
4. **The runtime pause boundary was violated.** The first pipeline task-projection write failed with `task_state_bootstrap_incomplete` and paused `codex-runtime:ZGV2L0VkaXRvckJQL2RlY2lzaW9uLW9z`. The same pipeline continued through three completed skills and launched `implementation-orchestrator` because pipeline settlement directly called `runNextPipelineSkill()` outside the paused global scheduler.
5. **The Control Room timer had a second independent defect.** Its renderer deliberately replaced the exact stopwatch with `Queued · waiting for execution` for queued work and `Codex Log · waiting` for waiting work. The metadata still called the same interval `executing`, producing the contradictory combination shown in the screenshot.
6. **Replication carried intent but not verified executor identity.** An active structural `executionIntent` suppressed node-local observations and cleared `executionNodeId` plus `executionNodeLabel`. Production projection always emitted `executionObservation: null`; the non-null observation paths exercised by federation tests were not produced by production code.
7. **The implementation was under-factored and over-modelled at the same time.** It had many representations of the same lifecycle but no shared transition kernel that owned their order. The complexity was therefore spent on reconciliation rather than on additional product capability.
8. **The runtime-incident task ownership defect was already corrected on the pulled revision.** Commit `5fdf4ff2` changed the target from the `admin` relative path to the stable Decision OS project ID. The live Control Room then reported `card-runtime-incident-review` under project `decision-os`, project ID `ZGV2L0VkaXRvckJQL2RlY2lzaW9uLW9z`. Source comments and operation names still incorrectly said `admin`.
9. **Run rejection was silent on the served responsive card route.** A paused Decision OS Codex runtime rejected both skill and pipeline admission with HTTP `503` and `runtime-scope-paused` before any run was created. The responsive detail view wrote that rejection into `.process-message`, while `setMobileCodexView()` kept `.process-message` hidden in the same detail view. The button was re-enabled with no visible explanation.
10. **The initial three active incidents reduced to two fault domains.** One stale Codex-runtime incident came from a temporary task-state convergence gate. Two federated-library incidents came from the same Mobile `browser` skill-snapshot timeout and were split only because the retry path recorded a different operation name.
11. **The incident policy turned transient readiness failures into indefinite subsystem outages.** Relay convergence and phone connectivity were healthy, but neither successful convergence nor reconnection resolved the persisted incidents. Codex admission and automatic federated-library synchronization therefore remained paused after their triggering conditions had passed.
12. **A post-deployment diagnostic found a fourth active incident in the same policy defect.** A `30`-minute Codex execution timeout was recorded twice as a project-wide `background:codex-runtime:*` pause. The child was terminated and its attempt could settle failed, but unrelated Decision OS execution remained blocked.
...
10. **Historical target — superseded:** This audit proposed deriving `interrupted` from a missing or expired observation. Epoch 4 keeps the durable lifecycle phase visible and persists `interrupted` only when startup recovery cannot match the exact child-process identity.
...
7. **Historical disconnect target — superseded:** Epoch 4 does not rewrite a durable active phase when an executor observation expires. Startup recovery owns the explicit process-identity check and resulting `interrupted` transition.
...
1. **Historical Epoch-3 authority:** `codex-executions.json` stored every attempt under the shared phase table. Epoch 4 removed this runtime authority and stores executions as replicated task-state entities.
...
6. **Historical Control Room behavior — superseded:** This implementation derived `interrupted` from expired active observations. Epoch 4 renders the durable replicated phase and lets startup recovery persist `interrupted` after an exact process-identity failure.
...
10. **Automated proof:** backend typecheck passed; frontend typecheck passed; relay typecheck passed; backend tests passed `368/368`; frontend tests passed `526/526`; relay tests passed `8/8`; `git diff --check` passed.
...
13. **Initial production revision:** implementation commit `ec58d130` was merged into `main` by `9294cc00` and pushed to `origin`. The workstation restarted on that revision; `/api/health` returned `ready`, every paused-component collection was empty, federation reconnected, Mobile was online with five projects, and all four historical incidents in section `S` were resolved.

---

## U. Activation Incidents Added to This Audit

1. **Terminal-pipeline migration defect:** the first workstation activation produced `76` canonical records: `71` succeeded, `1` failed, `1` cancelled, and `3` queued. All three queued records belonged to the historical cancelled pipeline `codex-pipeline-1783687748321-1210d27d`; their legacy skills were still marked `pending` because they had never launched.
2. **Root cause:** the terminal-pipeline migration branch returned every flattened legacy skill unchanged. `legacyPipelineRecord()` then mapped each never-launched `pending` descendant to canonical `queued`, although its parent pipeline was already terminal.
3. **Correction:** terminal pipelines now canonicalize never-launched pending descendants as `cancelled`. Startup also repairs already-migrated `preparing` and `queued` descendants when the matching legacy skill is pending under a terminal parent, preserving the execution ID and advancing the canonical revision through the transition kernel.
4. **Regression proof:** one test covers a fresh terminal-pipeline migration and one covers an idempotent repair of an existing canonical queued record. The complete backend suite passes `368/368` after the correction.
5. **Verification-harness incident:** the first complete backend rerun exhausted Node's `4 GB` heap in `federation-task-state-replicator.test.ts`. Its relay harness recursively scheduled every simulated socket frame through `queueMicrotask()`, so the independently migrated-node repair exchange starved timers and accumulated an unbounded microtask chain.
6. **Harness correction:** simulated relay delivery now uses `setImmediate()`, matching a socket's event-loop scheduling boundary. The federation replicator file passes `11/11`, and the subsequent complete backend run passes `368/368` in `49.75 s`.
7. **Remaining production gate:** merge and push the terminal-pipeline repair, restart the workstation once, require zero active canonical records from that cancelled pipeline, update Mobile to the same revision, restart Mobile once, then record cross-node canonical DTO equality and restart durability.
8. **Mobile pre-activation incidents:** Mobile was still at `bba187e2`, had no `codex-executions.json`, and reported four non-resolved incidents: `incident-ced34a48-4562-4895-a9f1-42aaaf6560e6` permanently paused federated-library synchronization after a Workstation `typography-system` HTTP `503`; `incident-ca9a67cb-835a-4e79-af8b-bf779b8ba5da`, `incident-f8dbe652-5579-4ddb-a6c9-0ef41213a2e6`, and `incident-a17f05c6-b0ad-42c4-bf9c-637241c3b5ff` retained stale `task_state_bootstrap_incomplete` failures for the `lys` project.
9. **Mobile history preservation:** Mobile contained local-only documentation commit `bba187e2` while `origin/main` was `26` commits ahead. Non-mutating merge-tree proof found no conflicts and produced tree `76b0b5dd8f5c63fcf74fb0019b74c55595606d8d`, byte-identical to workstation revision `ee2b6de0`. Mobile created merge `4e8df952`; its exact `1,186`-byte verified Git bundle was transferred through the authenticated node channel and pushed from the workstation with the required Wise SSH identity because that key is absent on Mobile. Workstation, Mobile, and `origin/main` then shared commit `4e8df952` without changing the merged tree.
10. **Node-message result-delivery incident:** two post-restart proof requests ran for approximately five minutes and then the workstation CLI returned only `fetch failed`. Federation remained connected, Mobile stayed online, the workstation had no active incident, and the request stream count remained exactly one until the client disconnected.
11. **Node-message root cause:** `decision-os-node.mjs` used Node's built-in `fetch()` while the server contract permits a remote Codex turn to run for `30` minutes. The underlying fetch client stopped waiting for final HTTP headers after approximately five minutes. The local HTTP close aborted the federation request, which then cancelled otherwise healthy Mobile work.
12. **Node-message correction:** the CLI now uses a bounded native HTTP/HTTPS request with explicit cancellation, a `32 MiB` response limit, `30 s` discovery timeout, and `31 min` message timeout. This exceeds the server's `30 min` execution contract without creating an unbounded wait. The focused CLI regression, syntax check, `git diff --check`, and complete backend suite `368/368` pass.
13. **Updated production gate:** activate the node-message client correction on the workstation, retrieve the definitive Mobile post-restart state, fast-forward Mobile to the same final revision, then record one real card-owned canonical execution observation on both nodes and its durable terminal state after restart.
