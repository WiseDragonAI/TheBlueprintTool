...
5. **Automated verification.** Frontend typecheck passes; all `520` frontend tests pass. Backend typecheck passes; all `339` backend tests pass. Focused tests cover immediate application, rollback, stale-response resistance, exact replica routing, held-state invalidation, zero held federation publication, continued HTTP availability after rejection, and non-pausing task-state bootstrap gates.
6. **Workstation runtime verification.** The registered workstation server was restarted on `2026-07-22` with merge commit `a6f9e315`. `/api/health` returned ready with zero incidents. The deleted incident card `card-c9a30d5a-4f75-48a8-b5a8-650fb51e83d1` was absent from both `/api/control-room` and the project task-state projection. A rejected lifecycle mutation for a nonexistent card returned `404`, after which `/api/health` remained ready with zero incidents.
7. **Failure-boundary runtime verification.** A temporary `task_state_bootstrap_incomplete` write gate had been persisted as active HTTP, watcher, and Codex-startup incidents. These paths now return `503` where applicable, retain resolved diagnostics, keep health ready, let the HTTP listener start, and retry write-required Codex recovery after relay convergence. The deployed retry cleared one stale execution lease without pausing a runtime component.
8. **Federation runtime verification.** The workstation is connected, the phone reports `online: true` with five projects, and the workstation Decision OS root equals the relay root `492ab68296e703a7f23c312844f9c5adea3fbffce61cd748bc22c4c236818dc0`. Runtime dirty state, pending deliveries, content queue depth, and active incidents are all zero. The workstation has not received a post-restart phone convergence row, so the phone must still report its exact Decision OS relay root.
9. **Operator-facing verification remains open.** The workstation browser still requires one disposable task deletion to prove immediate row removal before the request resolves. The phone surface requires the same interaction after its exact relay-root equality is reported.
