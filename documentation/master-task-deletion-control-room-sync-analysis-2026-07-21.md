...
7. **Operator-facing verification remains open.** The workstation browser still requires one disposable task deletion to prove immediate row removal before the request resolves. The phone node is registered with five projects but currently reports `online: false`, so phone-surface verification cannot run until its server reconnects to the relay.
