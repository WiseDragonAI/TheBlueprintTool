...
5. A same-dot/different-value collision pauses only its project repair scope, remains paused across reconnects, records the exact rejected entity, relay hash, field path, and causal dot, then converges through an explicit local-authority successor mutation without deleting relay state.
...
13. Every implementation hunk must map to one of six authorized recovery outcomes: stop startup capture storms, make unchanged repair work quiescent across reconnects, expose complete health without blocking contained-error deployment, reconcile each affected project without deletion, prove the existing release and delivery path in isolation, or settle a retained incident through its verified owning success transition.
...
9. Extend the existing `state-relay-ack` payload additively with a `rejected` array containing the exact entity key, incoming state hash, relay state hash, stable rejection code, and every conflicting field path and causal dot. Do not add a new frame type and do not change epoch 4.
...
11. Keep same-dot collision detection strict. Explicit runtime recovery reasserts only the preserved local candidate on each recorded conflicting path under one fresh deterministic local recovery dot whose context covers the rejected dot. The relay admits that valid epoch-4 causal successor through the ordinary entity batch path; no state is deleted, replaced out of band, or interpreted as empty.
12. Clear the project-scoped collision incident only after the correlated successor acknowledgment, exact relay and local root equality, zero dirty entities, zero pending deliveries, and persistence across restart. A changed relay generation cannot clear a collision incident.
13. Apply the same repair-admission and collision-incident contract generically to every registered project in `federation-relay/src/index.ts`, `federation-relay/src/termux-local-relay.ts`, `shared/federation-repair-guard.ts`, and `backend/src/business/federation/helper/federation-task-state-replicator.ts` without adding a new repair store or state enum.
14. Add adjacent `WHAT:` and `WHY:` comments to every added or modified branch.
15. Keep production promotion factorized as deployment. Make candidate, admission, journal, status, and final receipts retain the complete diagnostic counts without treating contained project failures as deployment failures. The command still rejects Git, credential, Worker, coordinator-supervisor, delivery-runtime, and fatal-process failures.
16. In `create-decision-os-server.ts`, resolve the existing `server-listener` incident only after the registered listener successfully binds its intended host and port. Keep the incident active on `EADDRINUSE` and require the subsequent health probe before recovery evidence is accepted.
...
7. Reuse the existing `decision-os-merge-dev`, paired-tag resolver, delivery state machine, delivery journal, node release store, Worker upload and activation helpers, connector, replicator, `TaskCurrentStateStore`, Worker relay, Termux relay, and runtime recovery route.
...
10. The harness never promotes, deletes or clears production state, restarts production, tags, pushes, resolves production incidents, connects a new node to production, or deploys against production. It prepares evidence for the separate authorized production procedure.
...
2. Record a SHA-256 inventory of the complete settings-owned master `.decision-os` tree, the complete `.decision-os` tree of every project in the version-`2` registry, and the complete settings-owned production release root. The registered projects include Ardaria, MOH, lys, Decision OS, admin, Search, Rudy, and AgentSocial. Record each source path, file count, byte count, mode, link text, and digest without printing secret contents.
...
4. Create one owner-only immutable snapshot archive containing every inventoried master, project, and release-authority byte, including settings, Git metadata, runtime evidence, delivery journals, caches, uploads, the active release pointer, immutable release trees, markers, and launchers. Preserve symlink text without dereferencing it. Derive runnable canary trees separately; never execute directly from the full archive.
...
6. Quarantine `.settings.json`, federation credentials, delivery journals, leases, caches, uploads, telemetry, Git metadata, process state, and production relay configuration outside the runnable canary trees while retaining them byte-identically in the immutable archive. Generate isolated settings, identities, credentials, ports, and release roots.
7. Inventory the complete live source again after copying. Accept the snapshot only when the two full source inventories and the full archive inventory match. Reconstruct the archived catalog and release topology in a non-running scratch restore and require its inventory, registry identities, project roots, active release pointer, marker, and launcher to match. A concurrent legitimate write rejects that attempt and causes a bounded retry; the harness never pauses production to obtain a snapshot.
...
11. Do not claim an exact production relay snapshot. The Worker exposes no read-only Durable Object export and Wrangler exposes no safe export command.
...
8. Run the deterministic same-dot collision. Require one correlated rejection with both hashes and exact field/dot evidence, one project-scoped runtime incident, preserved local bytes, and zero additional scans, entity frames, summaries, observer traffic, and outbound repair bytes across `20` unchanged reconnects.
9. Invoke explicit local-authority reconciliation for the collided project. Require one fresh causal successor for only the rejected field paths, ordinary relay join with no deletion, correlated acknowledgment, zero dirty entities, zero pending deliveries, exact per-project root equality, restart persistence, and uninterrupted healthy control-project traffic.
...
12. Before restoring `env.dev`, require its active version to equal the harness-owned deployed version. External version drift is a fail-closed cleanup result and must not overwrite another actor's deployment. With unchanged authority, disconnect both nodes, delete only the manifest-owned temporary canary federation as teardown, restore the recorded pre-run Worker version, and verify it is active. Temporary canary deletion is cleanup, never reconciliation evidence.
...
14. Retain the full snapshot inventory test, canonical merge sandbox test, tag-resolution test, exact tagged-tree and child-gitlink test, production-override rejection, byte-fixed production Wrangler argument test, dev Worker deployment test, interruption and rollback tests, watcher baseline and candidate tests, collision containment and causal reconciliation test, huge-state `env.dev` and Termux apply-and-reload tests, `20`-reconnect test, healthy control-project test, source-write rejection test, external-Worker-drift cleanup test, and manifest teardown test. Run every check through `node bin/decision-os-verify.mjs`.
...
1. Authorization is recorded by the operator's instruction to complete point 9 with a verified backup before state reconciliation and manifest-owned cleanup afterward. The recovery boundary is the production coordinator, the complete settings-owned master state, and every project in the authoritative version-`2` registry; it is not limited to MOH.
2. Record the exact production coordinator release, process identity, full registry, per-project task-state entity counts and roots, dirty keys, pending-delivery keys, active incidents, relay deployment ID, and every observed relay project root before mutation.
3. Freeze automatic reconnects and restarts of the old coordinator. Do not run the uncommitted acknowledgment-timeout retry worktree and do not reconnect the previous flood-prone binary after the fixed release takes ownership.
4. Treat deployment as the existing reversible tag operation. Do not perform causal reconciliation during candidate admission or deployment.
...
6. Keep the current coordinator online only for deploy-only admission and preparation. Do not force a federation reconnect and record its complete contained diagnostic state in the candidate receipt.
...
9. Let the fixed coordinator classify each existing collision once. Require one correlated project-scoped incident per affected project, exact structured collision evidence, and no repeated repair traffic. The deploy-only receipt may complete with this contained diagnostic state recorded explicitly.
10. Stop the fixed coordinator through its delivery-settings-owned supervisor. Verify its process has exited and automatic restart remains frozen.
11. Create a new owner-only backup directory on the external durable volume and run the fixed full-state backup command:

   ```bash
   node bin/decision-os-production-state-backup.mjs prepare \
     --backup-directory <absolute-new-external-directory> \
     --json
   ```

12. Independently run `verify` against the same directory. Require byte-identical inventories for the complete master `.decision-os` tree and all eight registered project `.decision-os` trees, including Ardaria, plus registry identity, incident evidence, active release authority, per-project task-state roots/counts, file modes, symlink text, file counts, byte counts, and SHA-256 roots. Retain this backup permanently; cleanup cannot delete it.
13. Restart only the fixed coordinator through the same adopted supervisor. Require the same fixed release identity and one paused collision scope per affected project before reconciliation.
14. For each affected project, invoke the existing explicit runtime recovery endpoint with the exact persisted scope and the fixed `reconcile-local-authority` resolution. The runtime writes one fresh causal successor for each recorded conflicting field through the ordinary epoch-4 store and relay batch path. No relay project, entity, bucket, credential, generation, or repair history is deleted.
15. Require every successor acknowledgment to correlate with its stored collision evidence, every affected local and relay root to match, all dirty and pending counts to reach zero, and the same roots to survive a coordinator restart.
16. Preserve the full backup, deployment journal, incident history, collision receipts, and reconciliation receipts. Remove only manifest-owned canary resources after final acceptance.
...
2. After the startup patch is active, resume every copied `project-watcher:<project-id>` scope through `POST /api/diagnostics/runtime/resume` with a resolution that names the validated project file inventory and patch release. This includes MOH, lys, Ardaria, and any additional registered project scope present at execution time.
...
6. Resume each `federation-repair:<project-id>` scope only after its local-authority causal successor has been acknowledged, its exact local and relay roots match, dirty and pending counts are zero, and equality persists across coordinator restart.
...
5. Require every registered project watcher, including MOH, lys, and Ardaria, to be in its verified owning state with no new capture, collision, permission, timeout, and port-conflict incident.
...
6. Prove candidate quiescence, incident recovery, non-destructive causal reconciliation, and full huge-state synchronization on Worker and Termux relays from the identical complete-state snapshot.
...
11. Bind state reconciliation to the recorded point-9 authorization and exact full-production-state backup receipt.
12. Deploy the fixed tag, stop the fixed coordinator, back up the complete master and all registered project state, restart the fixed coordinator, and reconcile each affected project through fresh causal successors without deleting relay state.
...
3. This document authorizes documentation and implementation planning only. The harness may use the isolated dev Worker target but may not mutate production. Production stopping, full-state backup, release promotion, causal reconciliation, and incident resumption retain their explicit gates above.
