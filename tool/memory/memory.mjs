...
const option = (name) => {
  ...
  return index >= 0 ? String(argumentsList[index + 1] ?? '').trim() : '';
};
...
if (command === 'add') {
  ...
  output = readMemories(root, {
    ...
    query: command === 'search' ? option('query') : '',
  });
  ...
  throw new Error('Usage: memory.mjs add|list|search|migrate --root <server-launch-root> [options]');
}
