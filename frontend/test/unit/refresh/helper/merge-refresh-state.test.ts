/**
 * WHAT: Unit test for implemented function merge-refresh-state.
...
test('merge-refresh-state executes implemented behavior and records telemetry', async () => {
  traces.length = 0;
  const runtime_state: Record<string, unknown> = {};
  const result = await mergeRefreshState({
