/**
 * WHAT: Unit test for implemented function handle-client-refresh-controller.
...
test('handle-client-refresh-controller executes implemented behavior and records telemetry', async () => {
  traces.length = 0;
  const runtime_state: Record<string, unknown> = {};
  const result = await handleClientRefreshController({
