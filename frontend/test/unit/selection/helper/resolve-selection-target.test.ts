/**
 * WHAT: Unit test for implemented function resolve-selection-target.
...
test('resolve-selection-target executes implemented behavior and records telemetry', async () => {
  traces.length = 0;
  const runtime_state: Record<string, unknown> = {};
  const result = await resolveSelectionTarget({
