/**
 * WHAT: Unit test for implemented function clear-transient-selection.
...
test('clear-transient-selection executes implemented behavior and records telemetry', async () => {
  traces.length = 0;
  const runtime_state: Record<string, unknown> = {};
  const result = await clearTransientSelection({
