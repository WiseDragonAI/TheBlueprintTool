/**
 * WHAT: Unit test for implemented function calculate-relationship-ports.
...
test('calculate-relationship-ports executes implemented behavior and records telemetry', async () => {
  traces.length = 0;
  const runtime_state: Record<string, unknown> = {};
  const result = await calculateRelationshipPorts({
