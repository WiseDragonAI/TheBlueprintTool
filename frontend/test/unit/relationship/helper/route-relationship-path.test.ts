/**
 * WHAT: Unit test for implemented function route-relationship-path.
...
test('route-relationship-path executes implemented behavior and records telemetry', async () => {
  traces.length = 0;
  const runtime_state: Record<string, unknown> = {};
  const result = await routeRelationshipPath({
