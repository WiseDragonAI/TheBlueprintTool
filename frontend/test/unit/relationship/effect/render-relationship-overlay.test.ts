/**
 * WHAT: Unit test for implemented function render-relationship-overlay.
...
test('render-relationship-overlay executes implemented behavior and records telemetry', async () => {
  traces.length = 0;
  const runtime_state: Record<string, unknown> = {};
  const result = await renderRelationshipOverlay({
