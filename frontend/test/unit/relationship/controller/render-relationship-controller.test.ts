/**
 * WHAT: Unit test for implemented function render-relationship-controller.
...
test('render-relationship-controller executes implemented behavior and records telemetry', async () => {
  traces.length = 0;
  const runtime_state: Record<string, unknown> = {};
  const result = await renderRelationshipController({
