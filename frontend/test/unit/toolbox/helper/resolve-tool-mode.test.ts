/**
 * WHAT: Unit test for implemented function resolve-tool-mode.
...
test('resolve-tool-mode executes implemented behavior and records telemetry', async () => {
  traces.length = 0;
  const runtime_state: Record<string, unknown> = {};
  const result = await resolveToolMode({
