/**
 * WHAT: Unit test for implemented function render-group-layer.
...
test('render-group-layer executes implemented behavior and records telemetry', async () => {
  traces.length = 0;
  const runtime_state: Record<string, unknown> = {};
  const result = await renderGroupLayer({
