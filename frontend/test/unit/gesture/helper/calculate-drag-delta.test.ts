/**
 * WHAT: Unit test for implemented function calculate-drag-delta.
...
test('calculate-drag-delta executes implemented behavior and records telemetry', async () => {
  traces.length = 0;
  const runtime_state: Record<string, unknown> = {};
  const result = await calculateDragDelta({
