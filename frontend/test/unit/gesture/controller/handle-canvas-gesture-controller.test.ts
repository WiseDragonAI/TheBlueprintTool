/**
 * WHAT: Unit test for implemented function handle-canvas-gesture-controller.
...
test('handle-canvas-gesture-controller executes implemented behavior and records telemetry', async () => {
  traces.length = 0;
  const runtime_state: Record<string, unknown> = {};
  const result = await handleCanvasGestureController({
