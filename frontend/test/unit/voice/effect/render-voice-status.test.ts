/**
 * WHAT: Unit test for implemented function render-voice-status.
...
test('render-voice-status executes implemented behavior and records telemetry', async () => {
  traces.length = 0;
  const runtime_state: Record<string, unknown> = {};
  const result = await renderVoiceStatus({
