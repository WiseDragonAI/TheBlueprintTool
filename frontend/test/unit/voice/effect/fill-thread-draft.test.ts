/**
 * WHAT: Unit test for implemented function fill-thread-draft.
...
test('fill-thread-draft executes implemented behavior and records telemetry', async () => {
  traces.length = 0;
  const runtime_state: Record<string, unknown> = {};
  const result = await fillThreadDraft({
