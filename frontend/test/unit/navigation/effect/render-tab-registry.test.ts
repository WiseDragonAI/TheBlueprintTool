/**
 * WHAT: Unit test for implemented function render-tab-registry.
...
test('render-tab-registry executes implemented behavior and records telemetry', async () => {
  traces.length = 0;
  const runtime_state: Record<string, unknown> = {};
  const result = await renderTabRegistry({
