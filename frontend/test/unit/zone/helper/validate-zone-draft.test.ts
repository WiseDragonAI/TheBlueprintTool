/**
 * WHAT: Unit test for implemented function validate-zone-draft.
...
test('validate-zone-draft executes implemented behavior and records telemetry', async () => {
  traces.length = 0;
  const runtime_state: Record<string, unknown> = {};
  const result = await validateZoneDraft({
