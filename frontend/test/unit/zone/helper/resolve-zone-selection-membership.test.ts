/**
 * WHAT: Unit test for implemented function resolve-zone-selection-membership.
...
test('resolve-zone-selection-membership executes implemented behavior and records telemetry', async () => {
  traces.length = 0;
  const runtime_state: Record<string, unknown> = {};
  const result = await resolveZoneSelectionMembership({
