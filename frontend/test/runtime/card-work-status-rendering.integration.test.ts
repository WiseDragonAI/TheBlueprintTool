...
test('ledger card chrome renders todo processing backlog and done workflow statuses', async () => {
  ...
  try {
    ...
    const backlog = patchLedgerCard({ id: 'card-backlog', status: 'backlog', title: 'Backlog', comment: { what: 'Backlog.' } }) as unknown as FakeElement;
    ...
    const backlogButton = renderLedgerCardStatusButton('card-backlog', 'backlog', 'backlog') as unknown as FakeElement;
    ...
    assert.equal(backlog.dataset.cardStatus, 'backlog');
    assert.equal(backlog.dataset.cardWorkStatus, 'backlog');
    assert.equal(backlogButton.dataset.nextStatus, 'todo');
