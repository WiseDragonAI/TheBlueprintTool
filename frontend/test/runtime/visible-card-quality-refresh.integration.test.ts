...
test('zoomed card media promotes the native carousel into an untransformed overlay', () => {
  ...
  assert.match(media, /if \(mediaOverlaySuspended\) \{[\s\S]*clearMediaOverlay\(overlay, \{ reconcilePromotedGeometry: false \}\);[\s\S]*return;/);
  assert.match(media, /if \(mediaOverlaySuspended\) return/);
  assert.match(media, /clearMediaOverlay\(undefined, \{ reconcilePromotedGeometry: false \}\)/);
  ...
  assert.doesNotMatch(media, /document\.createElement\('img'\)/);
