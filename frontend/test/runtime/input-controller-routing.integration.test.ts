...
test('browser inputs route ledger commands through runtime controllers before server effects', () => {
  ...
  assert.match(actionClick, /const url = projectLedgersPath\(state\.projectId\);[\s\S]*window\.open\(url, '_blank', 'noopener'\)/);
  ...
  assert.match(noteCreate, /persistPendingThreadMessage/);
  assert.match(noteCreate, /commitPendingThreadMessage/);
  ...
  const specsLedger = source('frontend/test/fixtures/sidebar-collapse-state-spec.json');
  ...
  const threadLogBinding = source('frontend/src/runtime/codex/effect/bind-thread-codex-run-log.ts');
  ...
  assert.match(threadLogBinding, /renderThreadCodexLogUpdate/);
  ...
  assert.match(cardDetailSkillRunPoller, /const text = pending \? \(stopping \? 'CANCELLING' : 'CANCEL'\) : \(stopping \? 'STOPPING' : 'STOP'\)/);
  assert.match(cardDetailSkillRunPoller, /'Stopping run'/);
  assert.match(cardDetailSkillRunPoller, /result\.error \|\| 'Stop failed'/);
  assert.match(cardDetailSkillRunPoller, /requestCardSkillRunContinue\(\{[\s\S]*projectId: poller\.projectId,[\s\S]*replicaNodeId: poller\.replicaNodeId,[\s\S]*ledgerId: poller\.ledgerId,[\s\S]*cardId: poller\.cardId,[\s\S]*runId: poller\.runId,[\s\S]*traceId,[\s\S]*codexModel,[\s\S]*codexEffort/);
  ...
  assert.match(cardDetailSkillRunPoller, /function paintExternallyStartedRun\(poller: Poller, status: 'pending' \| 'running' = 'pending', latestLabel = 'Submitting continuation'\): void \{[\s\S]*poller\.startedAtMs = Date\.now\(\);[\s\S]*poller\.element\.dataset\.runStatus = status;/);
  ...
  assert.match(objectsCss, /\.codex-run-widget\s*{[^}]*grid-template-columns:\s*minmax\(0, 1fr\) minmax\(108px, max-content\);[^}]*height:\s*132px;[^}]*overflow:\s*hidden;/s);
  assert.match(objectsCss, /\.codex-run-metadata\s*{[^}]*flex-wrap:\s*nowrap;[^}]*overflow:\s*hidden;/s);
  assert.match(objectsCss, /\.codex-run-continue,\s*\.codex-run-new-session\s*{[^}]*min-height:\s*22px;[^}]*font-size:\s*9px;/s);
  assert.match(objectsCss, /\.codex-run-stop\s*{[^}]*width:\s*58px;[^}]*height:\s*58px;[^}]*grid-template-rows:\s*28px 16px;/s);
