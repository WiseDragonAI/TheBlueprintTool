...
test('thread accent colors feed the voice widget graph and frame', () => {
  ...
  assert.match(responsiveThread, /renderThreadPanel\(\)/);
  assert.match(responsiveThread, /mobile-thread-inspector/);
  assert.doesNotMatch(responsiveThread, /thread-title-row/);
  assert.match(applicationCss, /mobile-thread-inspector \.thread-heading\s*{[^}]*grid-template-rows: 44px 44px;[^}]*gap: 12px;/);
  assert.match(applicationCss, /mobile-thread-inspector \.thread-heading\[data-codex-running="true"\]\s*{[^}]*grid-template-rows: 44px;[^}]*gap: 0;/);
