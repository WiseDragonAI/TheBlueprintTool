...
test('card markdown images render as resizeable aspect-preserving media and adjacent images become a carousel', () => {
  ...
  assert.match(css, /\.ledger-card-media-shell\s*{[^}]*--ledger-card-media-slider-row-height:\s*7px;[^}]*--ledger-card-media-title-row-min-height:\s*22px;[^}]*--ledger-card-media-title-slider-gap:\s*6px;[^}]*--ledger-card-media-bottom-inset:\s*8px;[^}]*max-width:\s*100%;[^}]*aspect-ratio:\s*var\(--ledger-card-media-aspect-ratio, 4 \/ 3\);[^}]*resize:\s*horizontal;/s);
  ...
  assert.match(css, /\.ledger-card-media-nav\s*{[^}]*top:\s*6px;[^}]*bottom:\s*calc\([^}]*var\(--ledger-card-media-title-row-min-height, 22px\)[^}]*\);/s);
