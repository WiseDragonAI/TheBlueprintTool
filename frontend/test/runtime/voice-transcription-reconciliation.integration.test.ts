...
test('targeted reconciliation retains its remote project after navigation leaves the project route', async () => {
  const restore = installRuntime();
  const previousFetch = globalThis.fetch;
  const previousLocation = globalThis.location;
  let requestedUrl = '';
  Object.defineProperty(globalThis, 'location', { configurable: true, value: new URL('http://decision-os.local/') });
  state.activeLedger.notes['thread-card-a'] = [{ id: 'note-a', message: 'Voice uploaded.', voiceFileRef: '/tmp/voice.wav', status: 'transcribing', revision: 2 }];
  globalThis.fetch = (async (url: string) => {
    requestedUrl = url;
    return { ok: true, status: 200, json: async () => ({ ok: true, note: { id: 'note-a', message: 'Remote transcript.', voiceFileRef: '/tmp/voice.wav', status: 'transcribed', revision: 4 } }) } as Response;
  }) as typeof fetch;
  try {
    await reconcileVoiceTranscription({ projectId: 'workstation:project-id', ledgerId: 'specs', threadId: 'thread-card-a', noteId: 'note-a' });
    assert.match(requestedUrl, /^\/p\/workstation%3Aproject-id\/api\/voice-transcription-status\?/);
  } finally {
    globalThis.fetch = previousFetch;
    if (previousLocation === undefined) delete (globalThis as { location?: Location }).location;
    else Object.defineProperty(globalThis, 'location', { configurable: true, value: previousLocation });
