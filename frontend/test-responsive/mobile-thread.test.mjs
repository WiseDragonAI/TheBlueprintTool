...
test('responsive card threads own desktop split geometry and documented entry shortcuts', () => {
  ...
  assert.match(shortcut, /parentMasterBack = document\.querySelector\('\.back-to-zone-button'\)\?\.dataset\.destination === 'parent-master-task'/);
  assert.match(shortcut, /key === 'escape' && desktop && \(currentCard\?\.labels\?\.includes\('master-task'\) \|\| parentMasterBack\)[\s\S]*querySelector\('\.back-to-zone-button'\)\?\.click\(\)/);
