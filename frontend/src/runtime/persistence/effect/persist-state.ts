...
export function persistState(): void {
  ...
  localStorage.setItem('corev2.canvas.state', JSON.stringify({ viewport: state.viewport, viewports: state.viewports, selection: state.selection, activeTab: state.activeTab, railCollapsed: state.railCollapsed, geometry: snapshotCanvasGeometry(), regionEdits: snapshotCanvasRegionEdits() }));
}
