...
export async function listPendingVoiceUploads(input: { ledgerId: string; threadId: string }): Promise<PendingVoiceUpload[]> {
  if (!indexedDbFactory()) {
    return Array.from(memoryStore.values(), cloneEntry).filter((entry) => entry.ledgerId === input.ledgerId && entry.threadId === input.threadId);
  }
  return runRequest<PendingVoiceUpload[]>('readonly', (store) => store.index(threadScopeIndex).getAll([input.ledgerId, input.threadId]));
}
