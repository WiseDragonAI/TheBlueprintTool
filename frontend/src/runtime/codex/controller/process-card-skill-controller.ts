...
export async function processCardSkillController(input: { cardId: string; skillName: string; contentKind?: CodexContentKind; codexModel?: string; codexEffort?: string }): Promise<boolean> {
  ...
  const result = await requestCardSkillProcess({ ledgerId, cardId: input.cardId, skillName: input.skillName, contentKind: input.contentKind, requestId, codexModel: input.codexModel, codexEffort: input.codexEffort });
  if (!result.ok) {
