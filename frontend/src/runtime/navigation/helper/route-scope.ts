...
export type RouteScope = {
  view: 'control-room' | 'done' | 'projects' | 'projects-canvas' | 'project' | 'ledgers' | 'ledger' | 'card' | 'settings' | 'skills' | 'pipelines' | 'unknown';
  projectId: string;
  ledgerId: string;
  zoneId: string;
  cardId: string;
...
export function routeScope(path: string): RouteScope {
  ...
  if (route[0] === 'done' && route.length === 1) return { view: 'done', ...empty };
