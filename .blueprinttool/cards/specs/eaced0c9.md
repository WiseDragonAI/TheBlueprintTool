Threads are stored as Markdown content files under `.blueprinttool/threads/<ledger-name>/<thread-id>.md`. Ledger JSON keeps `threadFiles` references, while runtime reads hydrate `notes` from those files before returning data to the browser or ledger-cli command handlers.
