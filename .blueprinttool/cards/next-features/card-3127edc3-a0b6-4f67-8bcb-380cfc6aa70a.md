...
1. **Goal.** Start a CoreV2 watcher station without requiring the operator to type `treat open notes` and without spending an initial inference turn.
...
1. **Cwd-scoped workspace.** The watcher resolves the active Blueprinttool workspace from the process cwd unless the operator explicitly names another workspace.
2. **No workspace hardcoding.** The hook and launcher must not bake in CoreV2, MOH, Ardaria, or any other specific directory.
