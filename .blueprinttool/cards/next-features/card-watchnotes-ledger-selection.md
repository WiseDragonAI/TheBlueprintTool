...
3. **Default workspace.** The watcher still resolves the Blueprinttool workspace from the process cwd.
...
1. **Primary source.** Read `.blueprinttool/state.json` and use its `tabs[]` entries as the canonical ledger list.
2. **Fallback source.** If state is missing or invalid, scan `.blueprinttool/*.json` and exclude local settings files such as `.settings.json`.
...
4. **Durable state.** Persist the selected ledger set in watcher-local state under `.blueprinttool`, along with the watcher run id and current status.
