...
2. **Skill docs.** Update `corev2-start-doc-server` and `corev2-treat-open-notes` skill copies if the watcher changes how sessions are started or how notes are processed.
...
3. **Durable operation.** Watcher state and logs are stored in the active workspace under `.blueprinttool`.
