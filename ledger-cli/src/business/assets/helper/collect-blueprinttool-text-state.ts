...
function normalizeBlueprinttoolPath(value: unknown): string | null {
  if (typeof value !== 'string' || !value.trim()) return null;
  const path = value.trim().replace(/^\.\//, '');
  if (path.startsWith('/.blueprinttool/')) return path.slice(1);
  if (path.startsWith('.blueprinttool/')) return path;
  return null;
}
...
function activeLedgerRefs(state: unknown): string[] {
  ...
  return state.tabs
    .filter(isRecord)
    .map((tab) => normalizeBlueprinttoolPath(tab.ledgerFile))
...
function referencedCardContentFiles(ledger: unknown): string[] {
  ...
  return ledger.cards
    .filter(isRecord)
    .map((card) => isRecord(card.comment) ? normalizeBlueprinttoolPath(card.comment.contentFile) : null)
...
function referencedThreadContentFiles(ledger: unknown): string[] {
  ...
  return Object.values(ledger.threadFiles)
    .map(normalizeBlueprinttoolPath)
    .filter((path): path is string => Boolean(path?.endsWith('.md')));
}

async function topLevelLedgerJsonFiles(workspaceRoot: string): Promise<string[]> {
  const blueprinttoolRoot = resolve(workspaceRoot, '.blueprinttool');
  ...
  return entries
    ...
    .map((entry) => `.blueprinttool/${entry.name}`)
    .filter((path) => path !== '.blueprinttool/state.json');
}

export async function collectBlueprinttoolTextState(input: { domain?: string; workspaceRoot: string }): Promise<{
  activeLedgerFiles: string[];
  referencedTextFiles: ClassifiedTextFile[];
  sourceFiles: string[];
  unusedTextFiles: ClassifiedTextFile[];
}> {
  const statePath = resolve(input.workspaceRoot, '.blueprinttool/state.json');
  const state = await exists(statePath) ? await readJson(statePath) : { tabs: [] };
  const activeLedgers = activeLedgerRefs(state)
    .filter((path) => !input.domain || path === `.blueprinttool/${input.domain}.json`);
  ...
  for (const ledgerRef of activeLedgers) {
    referencedTextFilePaths.set(ledgerRef, { kind: 'ledger-json', referencedBy: ['.blueprinttool/state.json'] });
  ...
  const allLedgerFiles = input.domain ? [`.blueprinttool/${input.domain}.json`] : await topLevelLedgerJsonFiles(input.workspaceRoot);
  const markdownRoots = input.domain
    ? [resolve(input.workspaceRoot, '.blueprinttool/cards', input.domain), resolve(input.workspaceRoot, '.blueprinttool/threads', input.domain)]
    : [resolve(input.workspaceRoot, '.blueprinttool/cards'), resolve(input.workspaceRoot, '.blueprinttool/threads')];
