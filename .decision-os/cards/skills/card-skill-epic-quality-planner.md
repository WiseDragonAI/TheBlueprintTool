## A. Source Wording

1. **Skill.** `epic/quality-planner`.
2. **Source group.** `DroidFactory`.
3. **Source wording.** Red-teams `Epic Architecture`, reduces over-engineering, corrects factorization, and defines quality gates before decomposition.

---

## B. Pipeline Fit

1. **Role.** Epic quality planner.
2. **Best use.** Use before `epic/decomposer`.

---

## C. Evidence Boundary

1. **Inventory source.** Reused from `.decision-os/cards/skills/card-9a1d7843-0bf4-47e8-b6df-cbc4e5571e39.md`.
2. **Terminology rule.** Keep the skill name, body structure names, labels, and pipeline stage names exactly as listed above when using this card.
