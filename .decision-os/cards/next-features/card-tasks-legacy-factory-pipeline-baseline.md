...
5. **decision-os mapping:** decision-os’s `Data` ledger is conceptually adjacent because it represents durable ledger objects, state, routes, canvas, cards, threads, events, generated reports, runtime data, worktrees, and test runs. The commercial model is a product-specific Business OS task/data model, not a GitLab workflow model.
