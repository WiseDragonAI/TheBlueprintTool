...
1. **The missing product is a Business OS decision/task cockpit.** The operator needs the commercial `business-os-mock` model translated into decision-os’s durable ledger system.
2. **The current decision-os card workflow is too loose for task operations.** Cards and threads can capture discussion, but they do not yet provide the structured `task_instance -> operator decision -> execution -> artifact -> next stage` loop from the commercial mock.
...
5. **It matches decision-os’s ledger philosophy:** decision-os can provide the durable object graph, routes, threads, events, generated reports, runtime data, and persisted UI state that the commercial mock needs.
...
5. **Make routes semantic.** decision-os should expose route-addressable `decisions`, `pipeline`, `pipeline/new`, `stages`, `workspace`, and `team` views rather than burying task state inside card threads.

---

## E. Acceptance Criteria

1. **Decision record:** A decision-os task record can render the same minimum decision card as the commercial mock: title, sections, recommendation, risk, draft, actions, and previous agent context.
