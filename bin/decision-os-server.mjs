...
function main() {
  ...
  if (process.argv.includes('--print-command')) {
    console.log(JSON.stringify({ node: process.execPath, args: ['--import', loader, server], env: { DECISION_OS_FRONTEND_ROOT: env.DECISION_OS_FRONTEND_ROOT, TSX_TSCONFIG_PATH: env.TSX_TSCONFIG_PATH }, cwd: process.cwd() }));
    return;
  }
