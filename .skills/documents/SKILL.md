...
On Codex desktop/macOS, request sandbox escalation when running `render_docx.py`. LibreOffice/`soffice` cannot run reliably inside the sandbox, so the render gate requires running the renderer outside the sandbox. Keep the render command prefix stable and narrow so users can approve just the renderer, not arbitrary Python execution; use `python render_docx.py ...` rather than shell wrappers or ad hoc environment prefixes.
...
# macOS/Codex desktop: request sandbox escalation for this stable renderer command prefix
