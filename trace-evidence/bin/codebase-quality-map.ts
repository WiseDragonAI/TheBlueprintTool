#!/usr/bin/env node
/**
 * WHAT: Maps a repository and queries AST control-flow rationale along source stack frames.
 * WHY: Agents need one static companion to Trace Evidence for understanding what calls what and why each branch exists.
 */
...
function renderMarkdown(report: QualityMap): string {
  ...
  const lines = ['# Codebase Quality Map', '', `Commit: \`${report.commit}\``, '', `Tracked files: ${report.files.length}`, `Applicable source files: ${applicable.length}`, `Functions: ${applicable.reduce((count, file) => count + file.functions.length, 0)}`, `Findings: ${report.findings.length}`, '', '## Files', ''];
...
async function analyze(): Promise<void> {
  ...
  const commit = execFileSync('git', ['-C', repository, 'rev-parse', '--verify', `${option('commit', 'HEAD')}^{commit}`], { encoding: 'utf8' }).trim();
  const output = resolve(option('output', join(repository, '.trace', 'quality-map', commit.slice(0, 12))));
  mkdirSync(output, { recursive: true });
  const temporary = mkdtempSync(join(tmpdir(), 'quality-map-'));
  const archive = join(temporary, 'snapshot.tar');
  const snapshot = join(temporary, 'snapshot');
  mkdirSync(snapshot);
  try {
    execFileSync('git', ['-C', repository, 'archive', '--format=tar', '--output', archive, commit], { timeout: 120_000 });
    execFileSync('tar', ['-xf', archive, '-C', snapshot], { timeout: 120_000 });
    const suppliedGraph = option('graph');
    const graphPath = /* WHAT: Reuse explicit graph evidence when supplied. WHY: Tests and cached analyses must not rerun Graphify. */ suppliedGraph ? resolve(suppliedGraph) : await runQualityGraphify(snapshot, output, Number(option('graphify-timeout-ms', '900000')));
    const lcov = option('lcov');
    const report = buildQualityMap({ repository, snapshot, commit, graphPath, lcovPath: /* WHAT: Join only an explicit coverage artifact. WHY: Missing coverage must remain unknown. */ lcov ? resolve(lcov) : undefined });
    ...
    writeFileSync(join(output, 'manifest.json'), `${JSON.stringify({ version: 1, commit, graphify: { package: 'graphifyy', version: '0.9.22', license: 'MIT' }, artifacts }, null, 2)}\n`);
    process.stdout.write(`${JSON.stringify({ commit, output, report: reportPath, files: report.files.length, functions: report.files.reduce((count, file) => count + file.functions.length, 0), findings: report.findings.length }, null, 2)}\n`);
  } finally {
...
async function main(): Promise<void> {
  ...
  process.stdout.write('Usage:\n  codebase-quality-map analyze --repo <path> [--commit <sha>] [--output <path>] [--lcov <path>] [--graph <graph.json>]\n  codebase-quality-map stack --report <quality-map.json> (--stack <text> | --stack-file <path>)\n  codebase-quality-map file --report <quality-map.json> --path <repository-path>\n');
}
