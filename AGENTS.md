...
- **Autonomous `dev` integration:** An agent is authorized to commit the iteration, merge its feature branch into `dev` with a merge commit, push `dev`, then remove the worktree and delete the merged feature branch without additional operator approval only after all of these gates pass:
  ...
  6. Before the parent merge, the canonical integration command admits local `dev` only when it equals fetched `origin/dev`, completes canonical dev initialization and cleanliness admission, then revalidates the exact published parent SHA. It binds the reviewed parent gitlink to feature-child `HEAD`, resolves the child source from the reviewed parent `.gitmodules` blob, publishes that exact child SHA with a lease on the observed source `dev` tip, and refetches equality. The caller must not publish the child.
  7. After the local merge, the fixed dev integration check accepts the exact reviewed feature SHA and proves that the new `.decision-os` gitlink descends from the first-parent gitlink, is fetchable from the reviewed child source, and is installed exactly in the persistent `dev` child checkout:
     ...
     ```
- **Failed gate:** A pre-merge child-publication rejection keeps `dev` unmerged plus the feature branch and worktree intact; report the exact rejection and do not publish, merge, push, or clean manually. A post-merge parent-admission failure keeps the local merge plus feature branch and worktree intact; report and repair the exact parent-admission condition, rerun the command, and do not push or clean up.
- **Mandatory successful-merge cleanup:** Run `node .worktrees/<feature-name>/bin/decision-os-worktree.mjs integrate <feature-name> --json` from outside the feature worktree. The command publishes the exact reviewed child SHA before the parent merge, emits the child-publication and parent-admission receipt, installs the child gitlink, runs the fixed admission check, pushes only the admitted SHA, removes the completed worktree, and deletes the merged feature branch. Delete all remaining iteration-temporary artifacts before reporting completion.
