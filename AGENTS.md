...
tail -n 200 "$HOME/.decision-os/frontend-telemetry.jsonl" | jq -c .
rg 'task-execution-http-settled|codex-log-summary-settled' \
  "$HOME/.decision-os/frontend-telemetry.jsonl"
```
...
- **Autonomous `dev` integration:** An agent is authorized to commit the iteration, merge its feature branch into `dev` with a merge commit, push `dev`, then remove the worktree and delete the merged feature branch without additional operator approval only after all of these gates pass:
  ...
  6. Before the parent merge, the canonical integration command admits local `dev` only when it equals fetched `origin/dev`, completes canonical dev initialization and cleanliness admission, then revalidates the exact published parent SHA. The feature worktree's `.decision-os` checkout and gitlink are disposable inputs: integration never publishes them and always restores the exact pre-merge dev gitlink before creating the merge commit.
  7. After the local merge, the fixed dev integration check accepts the exact reviewed feature SHA and proves that the `.decision-os` gitlink equals the first-parent dev gitlink, remains fetchable from the dev-reviewed child source, and is installed exactly in the persistent `dev` child checkout:
     ...
     ```
- **Failed gate:** A pre-merge parent-admission rejection keeps `dev` unmerged plus the feature branch and worktree intact. A post-merge parent-admission failure keeps the local merge plus feature branch and worktree intact. Report and repair the exact parent-admission condition, rerun the command, and do not push or clean up manually.
- **Mandatory successful-merge cleanup:** Run `node .worktrees/<feature-name>/bin/decision-os-worktree.mjs integrate <feature-name> --json` from outside the feature worktree. The command discards the feature's incoming `.decision-os` gitlink, retains canonical dev's exact gitlink, emits the resolution and parent-admission receipt, runs the fixed admission check, pushes only the admitted SHA, removes the completed worktree, and deletes the merged feature branch. Delete all remaining iteration-temporary artifacts before reporting completion.
...
- Feature implementation commits must not advance the parent `.decision-os` gitlink. Feature-worktree Decision OS card, thread, and runtime changes are disposable and are never delivered by parent integration; canonical dev retains its existing child state.
...
## JBot Resource Admission — Superseding Policy

All CPU-, RAM-, disk-, GPU-, or VRAM-intensive local work must enter the workstation-wide queue
through `~/.local/bin/jbot`. Preserve the documented Decision OS command shape and concurrency
limits inside `jbot run --cwd "$PWD" -- COMMAND...`; never directly launch an equivalent heavy
command while its run exists. JBot waits for the daemon-owned terminal receipt and returns the
managed command's exit code. If the client is interrupted, inspect `jbot status --active`; do not
submit a duplicate merely because output is quiet.

JBot owns one aggregate 80% builder budget. Learned CPU demand is admission accounting, not
a per-run core limit. Keep intentional lower concurrency in the existing CLI flags; JBot
...
the scheduler runs the command. Use `jbot history` for submitted/effective argv,
