...
export function createProjectRuntimeRegistry(input: {
  ...
  context: (decisionOsRoot: string, projectId: string) => ProjectRuntimeContext;
  dispose: (context: ProjectRuntimeContext) => void;
  startupTasks: Promise<void>[];
  tryContext: (project: DecisionOsProject, operation: string) => ProjectRuntimeContext | null;
} {
  ...
  const context = (activeDecisionOsRoot: string, projectId: string): ProjectRuntimeContext => {
  ...
  const tryContext = (project: DecisionOsProject, operation: string): ProjectRuntimeContext | null => {
    if (input.pausedProjectWatchers.has(project.id) || input.pausedProjectRuntimes.has(project.id)) return null;
    try {
      return context(project.decisionOsRoot, project.id);
    } catch (error) {
