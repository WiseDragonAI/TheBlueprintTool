...
export function createServerProjectRuntime(input: {
  ...
  const recovery = createRuntimeRecoveryService({
    ...
    migrateProjectPipelines,
    projectById: (projectId) => projectCatalog()
  ...
  const controlRoom = createControlRoomProjectionStore({
    ...
    runtimeForProject: (project) => projectRuntimeRegistry.contexts
      .get(project.decisionOsRoot)?.runtime,
    taskEntityForProject: (project, entityType, entityId) => localTaskRuntime
      .tryStateForProject(project)?.store.projectedEntity(entityType, entityId) ?? null,
    taskExecutionsForProject: (project) => localTaskRuntime
      .tryStateForProject(project)?.executions.all() ?? [],
    taskExecutionDiagnosticsForProject: (project) => localTaskRuntime
      .tryStateForProject(project)?.executions.diagnostics() ?? [],
    ...
    taskRootForProject: (project) => localTaskRuntime
      .tryStateForProject(project)?.store.rootHash() ?? `paused:${project.id}`,
  });
  connections.controlRoom = controlRoom;
  try {
    controlRoom.get(projectCatalog().filter((project) => project.available));
  } catch (error) {
    recordIncident({
  ...
  for (const project of projectCatalog()) {
    if (project.available) projectRuntimeRegistry.tryContext(project, 'start-project-runtime');
  }
  const reconcileProjectRuntimes = (): void => {
    const registered = projectCatalog();
    const activeRoots = new Set(registered.filter((project) => project.available)
  ...
  const incidentReviewScheduler = createRuntimeIncidentReviewScheduler({
    ...
    targetProject: () => projectCatalog()
      .find((project) => project.available && project.id === runtimeIncidentReviewProjectId) ?? null,
    ...
    onFailure: (error, context) => {
      recordBackgroundFailure(
        ...
        'synchronize-decision-os-project-master-task',
        error,
        context,
      );
    },
