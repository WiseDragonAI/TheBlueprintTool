...
export function createIncidentSupervisor(input: {
  ...
  for (const incident of input.incidentLedger.active()) {
    ...
    // WHAT: Hydrate the federation-only pause without removing its project store from runtime admission.
    // WHY: Repair containment must survive restart while unrelated project behavior remains online.
