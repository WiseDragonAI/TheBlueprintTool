...
export function createGlobalRequestHandler(input: {
  ...
  return (request: IncomingMessage, response: ServerResponse) => handleGlobalRequestStage({
    controlRoom: {
      controlRoomProjectionStore: input.controlRoom(),
      executionObservation: (projectId, executionId, ownerNodeId) => {
        const observation = input.executionObservations.get(
      ...
      hydrateProject: (project) => {
        input.executionRuntime.projectRuntimeRegistry.context(
    ...
    delivery: {
      ...
      dispatchRemote: (nodeId, command, signal) => input.federation.requestDelivery(
        nodeId,
        command,
      ...
      targetOnline: (nodeId) => input.federation.nodes()
    ...
    internalExecution: {
      ...
      authenticateNode: (nodeId) => input.federation.nodes()
        .some((node) => node.nodeId === nodeId && node.online),
      baseRuntime: (executionId, projectId) => {
        const project = projects().find((entry) => entry.id === projectId && entry.available);
        return input.federatedSchedulerContexts.get(executionId)?.runtime
          ?? (project
            ? input.executionRuntime.projectRuntimeRegistry.context(
              project.decisionOsRoot,
              project.id,
    ...
    markdown: {
      ...
      projects: projects(),
      taskLedger: (project) => (
        input.executionRuntime.localTaskRuntime.stateForProject(project).projection().ledger
      ),
    },
    ...
    nodeMessages: {
      ...
      projectRuntime: (project) => input.executionRuntime.projectRuntimeRegistry.context(
        project.decisionOsRoot,
        project.id,
      ).runtime,
    ...
    projectAdmission: {
      ...
      runtime: (project) => input.executionRuntime.projectRuntimeRegistry.context(
        project.decisionOsRoot,
        project.id,
      ).runtime,
    },
    ...
    remoteGateway: {
      ...
      federation: input.federation,
      invalidateProject: (projectId, changes) => input.controlRoom()
        .invalidate(projectId, [...changes]),
      localNodeId: input.federation.localOwner().ownerNodeId,
      masterDecisionOsRoot: input.masterDecisionOsRoot,
      pausedContentScheduler: () => input.incidentSupervisor.pausedBackgroundComponents
        .has('federation-content-scheduler'),
      ...
      presentationRuntime: (executionId) => (
      ...
      remoteProject: (ownerNodeId, projectId) => input.federation.remoteProjects()
        .find((project) => (
          project.ownerNodeId === ownerNodeId && project.localProjectId === projectId
        )) ?? null,
      replicator: input.federationState.replicator,
      revision: (projectId) => input.executionRuntime.federatedTaskRuntime
        .revisionForProject(projectId)
        .advance('tasks'),
