...
export async function handleGlobalRequestStage(input: {
  ...
  globalRuntimeIncident: () => { id: string } | null;
  ...
  const globalIncident = input.globalRuntimeIncident();
  if (globalIncident) throw new RuntimeScopePausedError('server-runtime', globalIncident.id);
