...
export async function handleThreadUploadRoutes(input: {
  ...
  if (input.url === '/api/thread-image-upload' && input.request.method === 'POST') {
    ...
    try {
      ...
      if (ledgerId === 'tasks' && input.localProject && cardId) {
        ...
        const delta = await input.taskStateForProject(input.localProject)
          .recordContentContribution(cardId, [imageFileRef, previewFileRef]);
