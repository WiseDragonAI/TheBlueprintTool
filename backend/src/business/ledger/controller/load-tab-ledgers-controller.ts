...
export async function loadTabLedgersController(input: { action_payload?: AnyRecord; runtime_state?: AnyRecord; data_model?: AnyRecord } | AnyRecord = {}): Promise<AnyRecord> {
  ...
  const state = readDecisionOsState({ action_payload: payload, runtime_state: runtime, data_model: data });
  ...
  writeDecisionOsState({ action_payload: { ...payload, state, ledger, validation }, runtime_state: runtime, data_model: data });
