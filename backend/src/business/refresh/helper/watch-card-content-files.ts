...
export function watchCardContentFiles(input: {
  ...
  function emitFile(file: string, kind: CardContentChange['kind']): void {
    ...
    pendingEvents.set(file, setTimeout(() => {
      ...
      if (!change) {
        // WHAT: Rebuild from the current Epoch 4 projection when a newly owned file emits its first event.
        // WHY: Post-migration task creation does not rewrite the legacy aggregate tasks ledger.
