...
export function createLocalTaskRuntime(input: {
  ...
  migrationAdmission: AnyRecord | null;
  migrationAdmissionBlocked: boolean;
  onExecutionChange: (
  ...
  const stateForProject = (project: DecisionOsProject): ProjectTaskState => {
    ...
    if (input.migrationAdmissionBlocked) {
      throw input.incidentSupervisor.pauseTaskProject(
        project,
        new Error(
          `task_migration_transaction_incomplete:${String(input.migrationAdmission?.phase ?? 'unknown')}`,
        ),
    ...
    try {
      ...
      throw input.incidentSupervisor.pauseTaskProject(project, error, 'open-local-task-state');
    }
  };
