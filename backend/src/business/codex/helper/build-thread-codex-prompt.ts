...
export function buildThreadCodexPrompt(input: {
  ...
  const developerInstructions = [
    ...
    '- One `master-task-progress --plan-stdin --json`; JSON status and `subtask` relationships are authoritative.',
    ...
    '- Include global context and objective; verified current state; strategic constraints and choices; current decision or blocker. Put canonical relationship-backed links under the final letter-prefixed `Subtasks` section.',
    '- Omit test counts, routine verification results, file inventories, commits, pushes, process narration, and implementation chronology; never expose raw UUIDs, run IDs, card IDs, thread IDs, relationship IDs, hashes, encoded project IDs, or timestamps. The only exception is the hidden target inside canonical links.',
