/**
 * WHAT: Unit test for implemented function read-ledger-json-file.
...
test('read-ledger-json-file executes implemented behavior and records telemetry', async () => {
  traces.length = 0;
  const runtime_state: Record<string, unknown> = {};
  const result = await readLedgerJsonFile({
