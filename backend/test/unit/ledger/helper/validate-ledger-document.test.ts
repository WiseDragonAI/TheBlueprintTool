/**
 * WHAT: Unit test for implemented function validate-ledger-document.
...
test('validate-ledger-document executes implemented behavior and records telemetry', async () => {
  traces.length = 0;
  const runtime_state: Record<string, unknown> = {};
  const result = await validateLedgerDocument({
