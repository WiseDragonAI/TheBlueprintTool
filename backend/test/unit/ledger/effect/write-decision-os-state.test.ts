/**
 * WHAT: Unit test for implemented function write-decision-os-state.
...
test('write-decision-os-state executes implemented behavior and records telemetry', async () => {
  traces.length = 0;
  const runtime_state: Record<string, unknown> = {};
  const result = await writeDecisionOsState({
