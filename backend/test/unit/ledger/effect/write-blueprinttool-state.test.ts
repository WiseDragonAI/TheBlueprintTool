/**
 * WHAT: Unit test for implemented function write-blueprinttool-state.
...
test('write-blueprinttool-state executes implemented behavior and records telemetry', async () => {
  traces.length = 0;
  const runtime_state: Record<string, unknown> = {};
  const result = await writeBlueprinttoolState({
