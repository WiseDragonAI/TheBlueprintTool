...
test('direct active master task uses the latest persisted Codex turn after server continuation', () => {
  ...
  try {
    const projection = store.get([project]) as Record<string, any>;
    assert.equal(projection.active.length, 1);
    assert.equal(projection.active[0].activeSince, '2026-07-14T10:12:03.000Z');
    assert.equal(projection.active[0].activeTime, Date.parse('2026-07-14T10:12:03.000Z'));
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});

test('direct active master task waits for Codex turn.started before starting its stopwatch', () => {
  ...
  try {
    const projection = store.get([project]) as Record<string, any>;
    assert.equal(projection.active[0].activeSince, '');
    assert.equal(Number.isNaN(projection.active[0].activeTime), true);
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});
