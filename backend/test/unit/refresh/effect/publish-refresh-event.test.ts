/**
 * WHAT: Unit test for implemented function publish-refresh-event.
...
test('publish-refresh-event executes implemented behavior and records telemetry', async () => {
  traces.length = 0;
  const runtime_state: Record<string, unknown> = {};
  const result = await publishRefreshEvent({
