/**
 * WHAT: Unit test for implemented function publish-server-refresh-controller.
...
test('publish-server-refresh-controller executes implemented behavior and records telemetry', async () => {
  traces.length = 0;
  const runtime_state: Record<string, unknown> = {};
  const result = await publishServerRefreshController({
