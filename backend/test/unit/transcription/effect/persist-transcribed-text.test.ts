/**
 * WHAT: Unit test for implemented function persist-transcribed-text.
...
test('persist-transcribed-text executes implemented behavior and records telemetry', async () => {
  traces.length = 0;
  const runtime_state: Record<string, unknown> = {};
  const result = await persistTranscribedText({
