...
test('independent offline fields converge without comparing unrelated counters', async (context) => {
  ...
  for (let counter = 0; counter < 12; counter += 1) {
    await a.value.mutate({ replicaId: 'desktop', changes: [{ entityType: 'card', entityId: `unrelated-${counter}`, changes: [{ path: 'status', operation: 'set', value: 'todo' }] }] });
  }
  ...
  const status = await b.value.mutate({ replicaId: 'mobile', changes: [{ entityType: 'card', entityId: 'shared', changes: [{ path: 'status', operation: 'set', value: 'done' }] }] });
...
test('concurrent same-field values remain bounded until an observed edit resolves them', async (context) => {
  ...
  context.after(async () => { await Promise.all([a.value.flush(), b.value.flush()]); rmSync(a.root, { recursive: true, force: true }); rmSync(b.root, { recursive: true, force: true }); });
  const left = await a.value.mutate({ replicaId: 'desktop', changes: [{ entityType: 'card', entityId: 'shared', changes: [{ path: 'status', operation: 'set', value: 'todo' }] }] });
  const right = await b.value.mutate({ replicaId: 'mobile', changes: [{ entityType: 'card', entityId: 'shared', changes: [{ path: 'status', operation: 'set', value: 'done' }] }] });
  ...
  const resolution = await a.value.mutate({ replicaId: 'desktop', changes: [{ entityType: 'card', entityId: 'shared', changes: [{ path: 'status', operation: 'set', value: 'blocked' }] }] });
  ...
  assert.equal((b.value.projection().ledger.cards as Array<Record<string, unknown>>)[0].status, 'blocked');
});
...
test('identical concurrent writes retain both causal origins for a later one-sided edit', async (context) => {
  ...
  context.after(async () => { await Promise.all([a.value.flush(), b.value.flush()]); rmSync(a.root, { recursive: true, force: true }); rmSync(b.root, { recursive: true, force: true }); });
  const left = await a.value.mutate({ replicaId: 'desktop', changes: [{ entityType: 'card', entityId: 'shared', changes: [{ path: 'status', operation: 'set', value: 'todo' }] }] });
  const right = await b.value.mutate({ replicaId: 'mobile', changes: [{ entityType: 'card', entityId: 'shared', changes: [{ path: 'status', operation: 'set', value: 'todo' }] }] });
  await a.value.merge(right.delta);
  assert.equal(a.value.projection().conflicts.length, 0);
  const oneSided = await b.value.mutate({ replicaId: 'mobile', changes: [{ entityType: 'card', entityId: 'shared', changes: [{ path: 'status', operation: 'set', value: 'done' }] }] });
  await a.value.merge(oneSided.delta);
  assert.equal(a.value.projection().conflicts.length, 1);
  assert.deepEqual(a.value.projection().conflicts[0].candidates.map((candidate) => candidate.value).sort(), ['done', 'todo']);
});
