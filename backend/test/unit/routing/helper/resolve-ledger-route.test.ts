/**
 * WHAT: Unit test for implemented function resolve-ledger-route.
...
test('resolve-ledger-route executes implemented behavior and records telemetry', async () => {
  traces.length = 0;
  const runtime_state: Record<string, unknown> = {};
  const result = await resolveLedgerRoute({
