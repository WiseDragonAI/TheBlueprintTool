/**
 * WHAT: Unit test for implemented function send-json-response.
...
test('send-json-response executes implemented behavior and records telemetry', async () => {
  traces.length = 0;
  const runtime_state: Record<string, unknown> = {};
  const result = await sendJsonResponse({
