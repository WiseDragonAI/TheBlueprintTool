...
test('content scheduler yields to priority state and installs only verified hashes', async (context) => {
  ...
  assert.equal(store.resource('node-a', 'project-a', '.decision-os/cards/tasks/card-a.md').state, 'synchronizing');
  priority = false;
  await scheduler.drain();
