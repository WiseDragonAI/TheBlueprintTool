/**
 * WHAT: Unit test for implemented function commit-ledger-edit-controller.
...
test('commit-ledger-edit-controller executes implemented behavior and records telemetry', async () => {
  traces.length = 0;
  const runtime_state: Record<string, unknown> = {};
  const result = await commitLedgerEditController({
