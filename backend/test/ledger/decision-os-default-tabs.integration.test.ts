...
test('Ledgers in .decision-os load as default tabs unless invalid', async () => {
  traces.length = 0;
  const expectedTelemetry = ["read-decision-os-state","read-ledger-json-file","validate-ledger-document","write-decision-os-state"];
  const argvPayload = {
    ok: true,
    mode: 'dry-run',
    apply_command: true,
  ...
  console.log(JSON.stringify({ specId: '9c31f0a4', suiteName: 'Ledgers in .decision-os load as default tabs unless invalid', controllerName: ["load-tab-ledgers-controller"], executionEntry: 'controller', expectedTelemetry, actualTelemetry }));
});
