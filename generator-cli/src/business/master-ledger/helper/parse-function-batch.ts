...
function toGeneratedFunction(kind: FunctionKind, domain: string, name: string, description: string, body = ''): GeneratedFunction {
  return {
    functionId: hash8(`${kind}:${domain}:${name}`),
    kind,
    domain,
    name,
    exportName: dashToCamel(name),
...
export function parseFunctionBatch(document: MasterLedgerDocument): FunctionBatch {
  ...
  const itemRegex = /type: '(helper|effect)'[\s\S]*?name: '([^']+)'[\s\S]*?description: '([^']*)'/g;
  let itemMatch: RegExpExecArray | null;

  while ((itemMatch = itemRegex.exec(helperSection)) !== null) {
    const [, rawKind, name, description] = itemMatch;
    const kind = rawKind as FunctionKind;
    const domain = domainFor(name, controlSection);
    functions.set(`${kind}:${name}`, toGeneratedFunction(kind, domain, name, description));
  }
