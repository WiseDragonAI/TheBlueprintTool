/**
 * WHAT: Integration test for spec ee77191d.
...
test('Node test runner executes generator-cli TypeScript unit tests', async () => {
  const expectedTelemetry = ["run-node-test"];
  assert.ok(expectedTelemetry.length > 0);
  assert.equal('ee77191d'.length, 8);
  traces.length = 0;
  ...
  console.log(JSON.stringify({
    specId: 'ee77191d',
    suiteName: 'Node test runner executes generator-cli TypeScript unit tests',
