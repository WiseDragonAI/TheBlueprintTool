...
import { fakeProcess, masterLedgerPath, specsLedgerPath, tempDir, readText } from '../fixture/scenario.js';
...
test('Apply mode writes generated worktree imports tests telemetry harness and report configuration', async () => {
  ...
  assert.ok((await readText(join(output, 'generator-cli/src/telemetry/harness.ts'))).includes('telemetry'));
  assert.ok((await readText(join(output, 'generator-cli/generated/test-results.json'))).includes('importCheck'));
});
