/**
 * WHAT: Integration test for spec a8b01160.
...
test('Unused function detection uses integration-test telemetry stack only', async () => {
  const expectedTelemetry = ["collect-telemetry-traces","infer-function-usage","detect-unused-functions"];
  assert.ok(expectedTelemetry.length > 0);
  assert.equal('a8b01160'.length, 8);
  traces.length = 0;
  ...
  console.log(JSON.stringify({
    specId: 'a8b01160',
    suiteName: 'Unused function detection uses integration-test telemetry stack only',
