/**
 * WHAT: Integration test for spec f2a91c7d.
...
test('Generator creates one source file per function', async () => {
  const expectedTelemetry = ["read-master-ledger","enumerate-generated-functions","write-source-file"];
  assert.ok(expectedTelemetry.length > 0);
  assert.equal('f2a91c7d'.length, 8);
  traces.length = 0;
  ...
  console.log(JSON.stringify({
    specId: 'f2a91c7d',
    suiteName: 'Generator creates one source file per function',
