/**
 * WHAT: Integration test for spec 3aec8ea9.
...
test('Auto-import during generation resolves imports without agent-specified paths', async () => {
  const expectedTelemetry = ["discover-dependency-references","resolve-import-paths"];
  assert.ok(expectedTelemetry.length > 0);
  assert.equal('3aec8ea9'.length, 8);
  traces.length = 0;
  ...
  console.log(JSON.stringify({
    specId: '3aec8ea9',
    suiteName: 'Auto-import during generation resolves imports without agent-specified paths',
